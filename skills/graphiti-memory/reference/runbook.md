...
## Качество записи (аудит 16.09.2026)

Что нашли за первый день: (1) 50 % фактов с `invalid_at` — штатный промпт `dedupe_edges.resolve_edge` считает уточнение
опровержением (`engineer → senior engineer`), а `custom_extraction_instructions` до дедупа НЕ доходит (только
`extract_nodes/edges` — проверено по коду 0.30.1, апстрим `main` идентичен); (2) склейка однофамильцев и нарицательных
(«банк» → Альфа-банк) — тот же промпт дедупа без наших правил; (3) секреты в графе: словарь фильтра отбрасывал цифровой
пароль, пароль в блоке кода не ловила ни одна регулярка, `--check` проверял фильтр им же самим.

| Задача | Инструмент |
|---|---|
| промпты дедупа/противоречий с нашими правилами (уточнение ≠ противоречие; однофамильцы/нарицательные → -1) | `prompt_patch.py` через `sitecustomize` (`GRAPHITI_PATCH=1`), действует и в MCP |
| умная модель на дедупе (в т.ч. MCP-путь, где фабрика ставит small=main) | `small_model_patch.py` + env `SMALL_MODEL_NAME=anthropic/claude-sonnet-5` |
| типы связей с семантикой «исключающая / накопительная», тип `Publication` | `config.yaml` → `edge_types`; загрузчики передают `edge_types=` (doc-only, без лишних вызовов) |
| общие правила извлечения (нарицательные не сущности, люди с ролью, план ≠ факт, TRIED_AND_FAILED) | `bulk_load.py: INSTR_COMMON` |
| дайджест сессии вместо сырого транскрипта (1–3 куска: решения, тупики с причиной, вопросы, правила) | `session_digest.py` (Sonnet 5, ≈$0.07/сессия), `state/digests/<sid>.md`, крон |
| метрики после докачки: быстрые погашения (<1 дня, норма <5 %), нарицательные карточки, хабы, $ по моделям | `metrics.py --since <T0>`; код 2 = алерт |
| учёт токенов по моделям (bulk идёт мимо прокси) | `bulk_load.make_clients` → `state/usage.jsonl` |
| ремонт ложных погашений в уже собранном графе | `repair_invalidations.py` (правило <1 дня + модель 1–7 дней, только при найденном факте-погасителе; откат в `state/repair_*.json`) |
| расклейка ошибочно слитой карточки по провенансу (эпизодам) | `split_node.py --episodes-match … --target …` |
| поиск карточек со «сшитой» сводкой | `mixed_scan.py` (flash-lite, ≈$0.08 на 2.6k карточек) |
| секреты: ключ с секретным именем → значение секрет всегда; `codeblock_pw`; `kv_ru` с контекстом; чистка и в `Community`; пересчёт эмбеддингов | `secret_filter.py`, `scrub_graph.py` |

🔴 Грабли: **никогда не подставлять значение секрета в текст Cypher-запроса** — FalkorDB пишет тексты запросов в поток
`telemetry{<graph>}`, и он попадает в RDB (нашли grep'ом по снимку). Сверять на стороне клиента, как `scrub_graph`.
`docker exec` без `-i` не передаёт stdin — heredoc-скрипт молча выполняется пустым. Сравнение дат строкой в Cypher
(`n.created_at >= '…Z'`) ненадёжно — парсить в Python. Тест на копии графа: после `GRAPH.COPY` выставить `group_id`
копии всем узлам/рёбрам, иначе дедуп «не видит» старые карточки (ищет только в своей группе).
Результат теста на копии: быстрые погашения 4.1 % (было 13–21 %), нарицательных 1 на 27 карточек.
